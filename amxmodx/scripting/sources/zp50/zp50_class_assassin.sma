...
public plugin_init()
{
	register_plugin("[ZP ROTD] Class: Assassin", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
{	
	new attacker = pev(weapon,pev_owner)
...
public native_class_assassin_set(plugin_id, num_params)
{
	...
	if (flag_get(g_Isassassin, id))
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Player already a assassin (%d)", id)
		return false;
	}
