...
public plugin_init()
{
	register_plugin("[ZP ROTD] Items Manager", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_items_register(plugin_id, num_params)
{
	...
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Can't register item with an empty name")
	...
	for (index = 0; index < g_ItemCount; index++)
	{
		...
		{
			log_error(AMX_ERR_NATIVE, "[ZP ROTD] Item already registered (%s)", name)
			return ZP_INVALID_ITEM;
		}
	}
...
public native_items_get_name(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid item id (%d)", item_id)
		return false;
	}
