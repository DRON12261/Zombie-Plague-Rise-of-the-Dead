...
new cvar_grenade_fire_duration, cvar_grenade_fire_damage, cvar_grenade_fire_slowdown, cvar_grenade_fire_hudicon, cvar_grenade_fire_explosion, cvar_grenade_fire_explode_impact

public plugin_init()
{
	register_plugin("[ZP ROTD] Grenade: Fire", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
	...
	cvar_grenade_fire_explode_impact = register_cvar("zp_grenade_fire_explode_impact", "1")
...
public fw_Grenade_Touch(iTouched, iToucher)
{
	static szClassName[20]; 
	pev(iTouched, pev_classname, szClassName, charsmax(szClassName))
    
	if (get_pcvar_num(cvar_grenade_fire_explode_impact))
	{
		if(equal(szClassName, "grenade"))
		{
			static usEvent; 
			usEvent = get_pdata_short(iTouched, m_usEvent)
