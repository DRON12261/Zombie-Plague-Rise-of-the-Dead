...
public plugin_init()
{
	register_plugin("[ZP ROTD] Admin Commands", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_admin_commands_zombie(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid Player (%d)", id_admin)
		return false;
	}
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid Player (%d)", player)
		return false;
	}
...
public _admin_commands_start_mode(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid game mode id (%d).", game_mode_id)
		return false;
	}
...
public cmd_zombie(id, level, cid)
{
	...
	if (zp_core_is_zombie(player))
	{
		...
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_ZOMBIE", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_human(id, level, cid)
{
	...
	if (!zp_core_is_zombie(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_HUMAN", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_nemesis(id, level, cid)
{
	...
	if (zp_class_nemesis_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_NEMESIS", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_assassin(id, level, cid)
{
	...
	if (zp_class_assassin_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_ASSASSIN", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_survivor(id, level, cid)
{
	...
	if (zp_class_survivor_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_SURVIVOR", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_sniper(id, level, cid)
{
	...
	if (zp_class_sniper_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "ALREADY_SNIPER", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_respawn(id, level, cid)
{
	...
	if (!allowed_respawn(id))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP ROTD] %L (%s).", id, "CANT_RESPAWN", player_name)
		return PLUGIN_HANDLED;
	}
...
public cmd_start_game_mode(id, level, cid)
{
	...
	if (!(0 <= game_mode_id < zp_gamemodes_get_count()))
	{
		client_print(id, print_console, "[ZP ROTD] %L (%d).", id, "INVALID_GAME_MODE", game_mode_id)
		return PLUGIN_HANDLED;
	}
