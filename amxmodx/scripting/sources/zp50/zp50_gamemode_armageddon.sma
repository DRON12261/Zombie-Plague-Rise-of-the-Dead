...
public plugin_precache()
{
	...
	register_plugin("[ZP ROTD] Game Mode: Armageddon", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public zp_fw_gamemodes_start()
{
	...
	new human_count = floatround(alive_count * get_pcvar_float(cvar_armageddon_ratio), floatround_ceil)
	new survivor_count = floatround(human_count * 0.5, floatround_ceil)
	new sniper_count = human_count - survivor_count
	new nemesis_count = alive_count - human_count - survivor_count
	new assassin_count = alive_count - human_count - nemesis_count
	...
	while (iNemesis < iMaxNemesis)
	{
		...
		if (zp_class_survivor_get(id) || zp_class_sniper_get(id) || zp_class_nemesis_get(id))
			continue;
	...
	while (iAssassins < iMaxAssassins)
	{
		...
		// Already a survivor or nemesis?
		if (zp_class_survivor_get(id) || zp_class_sniper_get(id) || zp_class_nemesis_get(id) || zp_class_assassin_get(id))
			continue;
