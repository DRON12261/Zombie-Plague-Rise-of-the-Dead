...
public plugin_init()
{
	register_plugin("[ZP ROTD] Core/Engine", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_core_infect(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Player already infected (%d)", id)
		return false;
	}
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid Player (%d)", attacker)
		return false;
	}
...
public native_core_cure(plugin_id, num_params)
{
	...
	if (!flag_get(g_IsZombie, id))
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Player not infected (%d)", id)
		return false;
	}
