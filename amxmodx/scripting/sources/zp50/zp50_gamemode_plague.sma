...
//new cvar_plague_nemesis_count, cvar_plague_survivor_count
new cvar_plague_nem_hp_multi, cvar_plague_surv_hp_multi
...
public plugin_precache()
{
	...
	register_plugin("[ZP ROTD] Game Mode: Plague", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public zp_fw_gamemodes_start()
{
	...
	new survivor_count = 1 //get_pcvar_num(cvar_plague_survivor_count)
	new nemesis_count = 1 //get_pcvar_num(cvar_plague_nemesis_count)
	new sniper_count = 2
	new assassin_count = 2
	new zombie_count = floatround((alive_count - (nemesis_count + survivor_count + sniper_count + assassin_count)) * get_pcvar_float(cvar_plague_ratio), floatround_ceil)
	...
	while (iAssassins < iMaxAssassins)
	{
		...
		// Already a survivor?
		if (zp_class_survivor_get(id) || zp_class_nemesis_get(id) || zp_class_sniper_get(id) || zp_class_assassin_get(id))
			continue;
	...
	if (get_pcvar_num(cvar_plague_show_hud))
	{
		...
		set_dhudmessage(HUD_EVENT_R, HUD_EVENT_G, HUD_EVENT_B, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
