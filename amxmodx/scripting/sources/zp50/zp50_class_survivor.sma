...
public plugin_init()
{
	register_plugin("[ZP ROTD] Class: Survivor", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_class_survivor_set(plugin_id, num_params)
{
	...
	if (flag_get(g_IsSurvivor, id))
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Player already a survivor (%d)", id)
		return false;
	}
