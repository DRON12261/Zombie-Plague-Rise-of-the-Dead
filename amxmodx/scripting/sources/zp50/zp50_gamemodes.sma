...
public plugin_init()
{
	register_plugin("[ZP ROTD] Game Modes Manager", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_gamemodes_register(plugin_id, num_params)
{
	...
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Can't register game mode with an empty name")
	...
	for (index = 0; index < g_GameModeCount; index++)
	{
		...
		{
			log_error(AMX_ERR_NATIVE, "[ZP ROTD] Game mode already registered (%s)", name)
			return ZP_INVALID_GAME_MODE;
		}
	}
...
public native_gamemodes_start(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid player (%d)", target_player)
		return false;
	}
...
public event_round_start()
{
	...
	{
		set_fail_state("[ZP ROTD] No game modes registered!")
		return;
	}
...
public choose_game_mode()
{
	...
	for (index = g_DefaultGameMode + 1; /*no condition*/; index++)
	{
		...
		else
		{
			...
			{
				remove_task(TASK_GAMEMODE)
				abort(AMX_ERR_GENERAL, "[ZP ROTD] Default game mode can't be started. Check server settings.")
				break;
			}
		}
	}
