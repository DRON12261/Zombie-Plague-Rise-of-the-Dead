...
public plugin_init()
{
	register_plugin("[ZP ROTD] Class: Nemesis", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (flag_get(g_IsNemesis, attacker) && !zp_core_is_zombie(victim))
	{
		...
		{
			// Set nemesis damage
			SetHamParamFloat(2, damage * get_pcvar_float(cvar_nemesis_damage))
			return HAM_HANDLED;
		}
	}
...
public native_class_nemesis_set(plugin_id, num_params)
{
	...
	if (flag_get(g_IsNemesis, id))
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Player already a nemesis (%d)", id)
		return false;
	}
