...
public plugin_init()
{
	register_plugin("[ZP ROTD] Class: Zombie", ZP_VERSION_STRING, "ZP Dev Team + DRON12261")
...
public native_class_zombie_set_next(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Invalid zombie class id (%d)", classid)
		return false;
	}
...
public native_class_zombie_register(plugin_id, num_params)
{
	...
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[ZP ROTD] Can't register zombie class with an empty name")
		return ZP_INVALID_ZOMBIE_CLASS;
	}
	...
	for (index = 0; index < g_ZombieClassCount; index++)
	{
		...
		{
			log_error(AMX_ERR_NATIVE, "[ZP ROTD] Zombie class already registered (%s)", name)
			return ZP_INVALID_ZOMBIE_CLASS;
		}
	}
